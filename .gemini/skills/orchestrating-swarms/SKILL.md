...
| **Team** | A named group of agents working together. One leader, multiple teammates. | `~/.gemini/teams/{name}/config.json` |
...
| **Task** | A work item with subject, description, status, owner, and dependencies. | `~/.gemini/tasks/{team}/N.json` |
| **Inbox** | JSON file where an agent receives messages from teammates. | `~/.gemini/teams/{name}/inboxes/{agent}.json` |
...
**Creates:**
- `~/.gemini/teams/feature-auth/config.json`
- `~/.gemini/tasks/feature-auth/` directory
...
**Removes:**
- `~/.gemini/teams/{team-name}/` directory
- `~/.gemini/tasks/{team-name}/` directory
...
// cat ~/.gemini/teams/code-review/inboxes/team-lead.json
...
cat ~/.gemini/teams/{team}/config.json | jq '.members[].backendType'
...
// Read ~/.gemini/teams/{team}/config.json
...
```bash
# Check team config
cat ~/.gemini/teams/{team}/config.json | jq '.members[] | {name, agentType, backendType}'

# Check teammate inboxes
cat ~/.gemini/teams/{team}/inboxes/{agent}.json | jq '.'

# List all teams
ls ~/.gemini/teams/

# Check task states
cat ~/.gemini/tasks/{team}/*.json | jq '{id, subject, status, owner, blockedBy}'

# Watch for new messages
tail -f ~/.gemini/teams/{team}/inboxes/team-lead.json
```
...
// cat ~/.gemini/teams/pr-review-123/inboxes/team-lead.json
...
```bash
cat ~/.gemini/teams/{team}/inboxes/team-lead.json | jq '.'
```
