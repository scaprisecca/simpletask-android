...
Brainstorming helps answer **WHAT** to build through collaborative dialogue. It precedes the ce-plan skill, which answers **HOW** to build it.
...
## Next Steps
[If `Resolve Before Planning` is empty: `→ the ce-plan skill for structured implementation planning]
[If `Resolve Before Planning` is not empty: `→ Resume the ce-brainstorm skill to resolve blocking questions before planning]
```
...
- **Proceed to planning (Recommended)** - Run the ce-plan skill for structured implementation planning
...
Immediately run the ce-plan skill in the current session. Pass the requirements document path when one exists; otherwise pass a concise summary of the finalized brainstorm decisions. Do not print the closing summary first.

**If user selects "Proceed directly to work":**

Immediately run the ce-work skill in the current session using the finalized brainstorm output as context. If a compact requirements document exists, pass its path. Do not print the closing summary first.
...
Recommended next step: the ce-plan skill
```
...
Resume with the ce-brainstorm skill when ready to resolve these before planning.
```
