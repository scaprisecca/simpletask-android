...
### Upload single file
```bash
rclone copy the path skill/to/file.mp4 remote:bucket/path/ --progress
```

### Upload directory
```bash
rclone copy the path skill/to/folder remote:bucket/folder/ --progress
```

### Sync directory (mirror, deletes removed files)
```bash
rclone sync the local skill/path remote:bucket/path/ --progress
```
...
```bash
rclone copy the path skill remote:bucket/ --dry-run
```
...
rclone copy the path skill remote:bucket/ --progress --retries=5
```
...
rclone check the local skill/file remote:bucket/file
