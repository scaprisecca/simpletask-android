...
| **Team** | A named group of agents working together. One leader, multiple teammates. | `~/.kiro/teams/{name}/config.json` |
...
| **Task** | A work item with subject, description, status, owner, and dependencies. | `~/.kiro/tasks/{team}/N.json` |
| **Inbox** | JSON file where an agent receives messages from teammates. | `~/.kiro/teams/{name}/inboxes/{agent}.json` |
...
**Creates:**
- `~/.kiro/teams/feature-auth/config.json`
- `~/.kiro/tasks/feature-auth/` directory
...
**Removes:**
- `~/.kiro/teams/{team-name}/` directory
- `~/.kiro/tasks/{team-name}/` directory
...
// cat ~/.kiro/teams/code-review/inboxes/team-lead.json
...
cat ~/.kiro/teams/{team}/config.json | jq '.members[].backendType'
...
// Read ~/.kiro/teams/{team}/config.json
...
```bash
# Check team config
cat ~/.kiro/teams/{team}/config.json | jq '.members[] | {name, agentType, backendType}'

# Check teammate inboxes
cat ~/.kiro/teams/{team}/inboxes/{agent}.json | jq '.'

# List all teams
ls ~/.kiro/teams/

# Check task states
cat ~/.kiro/tasks/{team}/*.json | jq '{id, subject, status, owner, blockedBy}'

# Watch for new messages
tail -f ~/.kiro/teams/{team}/inboxes/team-lead.json
```
...
// cat ~/.kiro/teams/pr-review-123/inboxes/team-lead.json
...
```bash
cat ~/.kiro/teams/{team}/inboxes/team-lead.json | jq '.'
```
