...
```bash
the ce-compound skill                    # Document the most recent fix
the ce-compound skill [brief context]    # Provide additional context hint
```
...
**Always run full mode by default.** Proceed directly to Phase 1 unless the user explicitly requests compact-safe mode (e.g., the ce-compound skill --compact` or "use compact mode").
...
- the ce-compound-refresh skill plugin-versioning-requirements`
- the ce-compound-refresh skill payments`
- the ce-compound-refresh skill performance-issues`
- the ce-compound-refresh skill critical-patterns`
...
re-run the compound skill in a fresh session.
```
...
- **Manual trigger**: User can invoke agents after the ce-compound skill completes for deeper review

## Related Commands

- the research skill [topic]` - Deep investigation (searches docs/solutions/ for patterns)
- the ce-plan skill - Planning workflow (references documented solutions)
