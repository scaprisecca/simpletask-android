...
Then proceed to Step 1 normally. The skill works from any environment as long as `~/.kiro/` (or `$CLAUDE_CONFIG_DIR`) exists on the machine.
...
**This project only** -- pass the project slug (absolute path with every non-alphanumeric char replaced by `-`, e.g., the users skill/tmchow/Code/my-project` becomes `-Users-tmchow-Code-my-project`):
```bash
...
1. **Apply all to user settings** (`~/.kiro/settings.json`)
2. **Apply all to project settings** (`.kiro/settings.json`)
3. **Skip**
...
```
Applied N rules to ~/.kiro/settings.json
Applied M rules to .kiro/settings.json

These commands will no longer trigger permission prompts.
```

If `.kiro/settings.json` was modified and is tracked by git, mention that committing it would benefit teammates.

## Edge Cases

- **No project context** (running outside a project): Only offer user-level settings as write target.
- **Settings file doesn't exist**: Create it with `{ "permissions": { "allow": [] } }`. For `.kiro/settings.json`, also create the `.kiro/` directory if needed.
