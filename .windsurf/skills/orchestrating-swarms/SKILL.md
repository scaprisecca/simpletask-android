...
| **Team** | A named group of agents working together. One leader, multiple teammates. | `~/.codeium/windsurf/teams/{name}/config.json` |
...
| **Task** | A work item with subject, description, status, owner, and dependencies. | `~/.codeium/windsurf/tasks/{team}/N.json` |
| **Inbox** | JSON file where an agent receives messages from teammates. | `~/.codeium/windsurf/teams/{name}/inboxes/{agent}.json` |
...
```
~/.codeium/windsurf/teams/{team-name}/
...
~/.codeium/windsurf/tasks/{team-name}/
├── 1.json                   # Task #1
├── 2.json                   # Task #2
└── 3.json                   # Task #3
```
...
**Creates:**
- `~/.codeium/windsurf/teams/feature-auth/config.json`
- `~/.codeium/windsurf/tasks/feature-auth/` directory
...
**Removes:**
- `~/.codeium/windsurf/teams/{team-name}/` directory
- `~/.codeium/windsurf/tasks/{team-name}/` directory
...
`~/.codeium/windsurf/tasks/{team-name}/1.json`:
```json
{
  "id": "1",
  "subject": "Review authentication module",
...
// cat ~/.codeium/windsurf/teams/code-review/inboxes/team-lead.json
...
cat ~/.codeium/windsurf/teams/{team}/config.json | jq '.members[].backendType'
...
// Read ~/.codeium/windsurf/teams/{team}/config.json
...
```bash
# Check team config
cat ~/.codeium/windsurf/teams/{team}/config.json | jq '.members[] | {name, agentType, backendType}'

# Check teammate inboxes
cat ~/.codeium/windsurf/teams/{team}/inboxes/{agent}.json | jq '.'

# List all teams
ls ~/.codeium/windsurf/teams/

# Check task states
cat ~/.codeium/windsurf/tasks/{team}/*.json | jq '{id, subject, status, owner, blockedBy}'

# Watch for new messages
tail -f ~/.codeium/windsurf/teams/{team}/inboxes/team-lead.json
```
...
// cat ~/.codeium/windsurf/teams/pr-review-123/inboxes/team-lead.json
...
```bash
cat ~/.codeium/windsurf/teams/{team}/inboxes/team-lead.json | jq '.'
```
